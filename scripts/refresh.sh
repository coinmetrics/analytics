...
# lisk
$PSQL -qc "\\pset footer off" -c "SELECT SUBSTRING(\"date\"::TEXT FOR 10) \"date\", \"cnt\" \"txCount\", \"value\" \"txVolume\", \"fees\" \"fees\", \"from_cnt\" \"fromAddrCount\", \"to_cnt\" \"toAddrCount\", \"addr_cnt\" \"addrCount\", \"med_value\" \"medTxVolume\", \"med_fees\" \"medFees\" FROM lisk_stats ORDER BY \"date\"" -A -F "," -o "lisk.csv" lisk_main lisk
