...
# neo
$PSQL -qc "\\pset footer off" -c "SELECT SUBSTRING(\"date\"::TEXT FOR 10) \"date\", \"cnt\" \"txCount\", \"value\" \"txVolume\", \"fees\" \"fees\", \"from_cnt\" \"fromAddrCount\", \"to_cnt\" \"toAddrCount\", \"addr_cnt\" \"addrCount\", \"med_fees\" \"medFees\", \"med_value\" \"medValue\" FROM neo_stats WHERE \"asset\" = '\\x602c79718b16e442de58778e148d0b1084e3b2dffd5de6b7b16cee7969282de7' ORDER BY \"date\"" -A -F "," -o "neo_gas.csv"
$PSQL -qc "\\pset footer off" -c "SELECT SUBSTRING(\"date\"::TEXT FOR 10) \"date\", \"cnt\" \"txCount\", \"value\" \"txVolume\", \"fees\" \"fees\", \"from_cnt\" \"fromAddrCount\", \"to_cnt\" \"toAddrCount\", \"addr_cnt\" \"addrCount\", \"med_fees\" \"medFees\", \"med_value\" \"medValue\" FROM neo_stats WHERE \"asset\" = '\\xc56f33fc6ecfcd0c225c4ab356fee59390af8560be0e930faebe74a6daff7c9b' ORDER BY \"date\"" -A -F "," -o "neo_neo.csv"

# lisk
$PSQL -qc "\\pset footer off" -c "SELECT SUBSTRING(\"date\"::TEXT FOR 10) \"date\", \"cnt\" \"txCount\", \"value\" \"txVolume\", \"fees\" \"fees\", \"from_cnt\" \"fromAddrCount\", \"to_cnt\" \"toAddrCount\", \"addr_cnt\" \"addrCount\", \"med_value\" \"medValue\", \"med_fees\" \"medFees\" FROM lisk_stats ORDER BY \"date\"" -A -F "," -o "lisk.csv" lisk_main lisk
